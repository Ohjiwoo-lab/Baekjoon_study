...
## 간단한 문제 풀이
반례에 대해서 생각하지 못한 부분!   
- 만약 파닭 개수보다 많이 파를 만들었다면? 필요없는 부분에 대해서는 라면에 넣을 수 있으므로, 추가적으로 더해줘야한다는 것!
- 코테에서는 반례를 알아서 찾아야하니까, 이런 것을 미리 생각하는 습관을 기르자.
- 항상 값이 0인 경우, 1인 경우는 특수하므로 따로 생각해주고, 조건을 좀 더 유심히 살펴보자.
